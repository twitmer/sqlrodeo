...
public class TestObjectFactory {
	...
	/**
	 * Test method for
	 * {@link org.sqlrodeo.action.ObjectFactoryAction#execute(org.sqlrodeo.ExecutionContext)}
	 * . This is a happy path test where the object is found via JNDI lookup, so
	 * there's no need to invoke the ObjectFactory.
	 * 
	 * @throws Exception
	 */
	@Test
	public void testExecute_fromJNDI_happyPath() throws Exception {
		...
		Element node = XmlFactory
				.build("<objectFactory id=\"myDataSource\" name=\"someName\" factoryClassName=\"ignoredForJndi\" objectClassName=\"ignoredForJndi\" ></objectFactory>")
				.getDocumentElement();
		...
		EasyMock.expect(mockContext.lookup("java:comp/env/someName"))
				.andReturn(theJndiObject);
		EasyMock.replay(mockContext);

		// Given: An ObjectFactoryAction based on the XML node and mock JNDI
		// context
	...
	public void testExecute_fromJNDI_noJndi() throws Exception {
		...
		Element node = XmlFactory
				.build("<objectFactory id=\"myDataSource\" factoryClassName=\"ignoredForJndi\" objectClassName=\"ignoredForJndi\" ></objectFactory>")
				.getDocumentElement();
	...
	public void testExecute_jndi_errors() throws Exception {
		...
		// Given: an XML node for the ObjectFactoryAction WITH a JNDI name that
		// won't resolve.
		Element node = XmlFactory
				.build("<objectFactory id=\"myDataSource\" name=\"someName\" factoryClassName=\"ignoredForJndi\" objectClassName=\"ignoredForJndi\" ></objectFactory>")
				.getDocumentElement();
		...
		try {
			...
			// Then: Action falls back to instantiating the specified
			// ObjectFactory class.
			assertTrue(e.getMessage().equals("ignoredForJndi"));
		}
	...
	public void testExecute_objectFactory() throws Exception {
		...
		// Given: an XML node for the ObjectFactoryAction WITHOUT a JNDI name.
		Element node = XmlFactory
				.build("<objectFactory id=\"myDataSource\" factoryClassName=\"org.sqlrodeo.action.TestObjectFactory$MockObjectFactory\" objectClassName=\"String\"></objectFactory>")
				.getDocumentElement();
		...
		System.out.println("WANT: "
				+ new MockObjectFactory().getClass().getName());
		// Given: An ObjectFactoryAction based on the XML node and mock JNDI
		// context
		Action action = new ObjectFactoryAction(node);
		action.validate();
