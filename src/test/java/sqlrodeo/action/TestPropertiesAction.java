...
public class TestPropertiesAction {
    ...
    public void testValidate() throws SAXException, IOException, ParserConfigurationException {
        ...
        Element node = XmlFactory.build("<properties>a=10</properties>").getDocumentElement();
        new PropertiesAction(node).validate();

        // Happy path, empty properties content.
        node = XmlFactory.build("<properties></properties>").getDocumentElement();
        new PropertiesAction(node).validate();

        // Happy path, href only, no content.
        node = XmlFactory.build("<properties href=\"stuff.properties\"/>").getDocumentElement();
        new PropertiesAction(node).validate();

        // Sad path: Combining Href & Content is not allowed.
        try {
            node = XmlFactory.build("<properties href=\"stuff.properties\">invalid=content</properties>").getDocumentElement();
            ...
            assertTrue(e.getMessage().contains("cannot have content and an href"));
    ...
    public void testValidate_sad_path_WithHrefAndContent() {
        ...
        try {
            Element node = XmlFactory.build("<properties href=\"stuff.properties\">invalid=content</properties>").getDocumentElement();
    ...
    /**
     * Test method for {@link sqlrodeo.action.PropertiesAction#execute(sqlrodeo.ExecutionContext)} .
     * 
     * @throws Exception
     */
    @Test
    ...
    public void testExecute_withValidHref() throws Exception {
        try {
            ...
            Element node = XmlFactory.build("<properties href=\"empty.properties\"/>").getDocumentElement();
            Action action = new PropertiesAction(node);
    ...
    public void testExecute_withInvalidHref() throws Exception {
        try {
            ...
            // refers to a properties file that lives in the same
            // package as this unit test file.
            Element node = XmlFactory.build("<properties href=\"invalid.properties\"/>").getDocumentElement();
            Action action = new PropertiesAction(node);
