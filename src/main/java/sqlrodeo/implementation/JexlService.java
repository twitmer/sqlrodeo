...
final public class JexlService {
    ...
    public Object evaluate(String expressionString, ExecutionContext context) throws JexlEvaluationException {
        ...
        try {
            ...
            Object result = jexl.createExpression(expressionString).evaluate(jexlContext);

            if(log.isDebugEnabled()) {
                log.debug(String.format("evaluate: expressionString, result, resultClass".replaceAll(", ", "=%s, ") + "=%s",
                        expressionString, result, (result != null ? result.getClass().getName() : "null")));
            }
            return result;
    ...
    public Object script(String script, ExecutionContext context) throws JexlEvaluationException {
        if(log.isDebugEnabled()) {
        ...
        try {
            ...
            if(log.isDebugEnabled()) {
                log.debug(String.format("script: script, result, resultClass".replaceAll(", ", "=%s, ") + "=%s", script, result,
                        (result != null ? result.getClass().getName() : "null")));
            }
            return result;
        } catch(JexlException je) {
            throw new JexlEvaluationException("Failed to execute: " + script, je);
        }
    }
    ...
    public String substitute(String source, ExecutionContext context) throws JexlEvaluationException {
        if(log.isDebugEnabled()) {
        ...
        try {
            ...
            while(matcher.find()) {
                ...
                Object result = jexl.createExpression(term).evaluate(jexlContext);
                ...
                // does not expand
                // regexes.
                String newWorkingSource = workingSource.replace("${" + term + "}", result.toString());

                // log.debug("New workingSource = " + workingSource);

                if(newWorkingSource.equals(workingSource)) {
                    throw new JexlEvaluationException("Failed to replace " + term + " with " + result + " in " + workingSource);
                }
