...
public final class ExecutionContext implements IExecutionContext {
	...
	if (log.isDebugEnabled()) {
	    log.debug(String.format(
		    "executeCloseActions: closeActions".replaceAll(", ",
			    "=%s, ") + "=%s", closeActions.size()));
	}
	...
	while (closeActions.size() > 0) {
		...
		log.debug("Ignoring error when executing closeAction ("
			+ closeAction + "): " + e.getMessage());
	    }
	}
    }
    ...
    /**
     * Find the default connection in the context. This only works if there is
     * only one Connection in the context.
     * 
     * @param context
     * @return
	...
	if (connections.size() == 1) {
	    ...
	    throw new NotFoundException(
		    "Could not identify default connecton from collection of "
	...
	int result = 1;
	result = prime * result
		+ ((closeActions == null) ? 0 : closeActions.hashCode());
	result = prime * result
